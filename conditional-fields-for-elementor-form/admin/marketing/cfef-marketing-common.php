...
if (! class_exists('CFEF_Marketing_Controllers')) {

	class CFEF_Marketing_Controllers
	{
		...
		public function __construct() {
			...
			if(!defined("formdb_marketing_submission")){
				...
				if(!in_array( 'sb-elementor-contact-form-db/sb_elementor_contact_form_db.php', $active_plugins ) && !get_option('cfef_formdb_marketing_dismissed', false)) {
					add_action('admin_enqueue_scripts', [$this, 'cfef_formdb_marketing_script']);
		
					add_action('in_admin_header', array($this, 'cfef_admin_notice_for_formsdb'));
				}
			...
			if ( in_array( 'elementor-pro/elementor-pro.php', $active_plugins ) || in_array('pro-elements/pro-elements.php', $active_plugins) || in_array( 'hello-plus/hello-plus.php', $active_plugins )) {
				...
				if (class_exists('acf_pro') && !in_array('loop-grid-extender-for-elementor-pro/loop-grid-extender-for-elementor-pro.php', $active_plugins, true)) {
                    add_action('elementor/element/loop-grid/section_query/before_section_end', [$this, 'cfef_add_acf_repeater_mkt_query_controls']);
                }
				...
				if (empty(array_intersect($required_plugins, $active_plugins))) {
					...
					add_action('elementor/element/form/section_form_fields/before_section_end', [$this, 'cfef_marketing_controls'], 100, 2);
				}
				if(!in_array('loop-grid-extender-for-elementor-pro/loop-grid-extender-for-elementor-pro.php', $active_plugins, true)){
                    add_action("elementor/element/taxonomy-filter/section_taxonomy_filter/before_section_end", [$this, 'cfef_register_controls'], 10);
                }
			}

			if (!is_plugin_active('timeline-widget-addon-for-elementor/timeline-widget-addon-for-elementor.php')) {

				add_action('admin_notices', array($this, 'cfef_show_tec_active_notice'));
			}



			add_action('wp_ajax_cfef_install_plugin', [$this, 'cfef_install_plugin']);

          	add_action('wp_ajax_cfef_mkt_dismiss_notice', [$this,'cfef_dismiss_notice_callback']);
		...
		public function formdb_plugin_install_button(){
            ...
            if ( $screen && 'elementor_page_e-form-submissions' === $screen->id ) {
                ...
                $button_text = __('Save To Google Sheet - Install Plugin','conditional-fields-for-elementor-form');
				$nonce = wp_create_nonce('cfef_install_nonce');
                
                $custom_js = "
                    jQuery(document).ready(function($) {

                        var button = '<a data-nonce=\"{$nonce}\" data-plugin=\"form-db\" target=\"_blank\" class=\"button button-primary cfef-install-plugin\">{$button_text}</a>';
		...
		public function cfef_formdb_marketing_script($page) {
			...
			wp_register_script(
				'cfef-formdb-marketing-js',
				CFEF_PLUGIN_URL . 'admin/marketing/js/cfef-form-marketing.js',
				['jquery'],
				CFEF_VERSION,
				true
			);

			wp_enqueue_script('cfef-formdb-marketing-js');
			...
			wp_localize_script(
				...
				[
					...
					'formdb_dismiss_nonce' => wp_create_nonce('cfef_dismiss_nonce_formdb_notice'),
		...
		public function cfef_admin_notice_for_formsdb()
		{
			...
			$notice_options = [
				...
				'button_secondary' => [
					'text' => esc_html__('Install Plugin','conditional-fields-for-elementor-form'),
					'classes' => ['cfef-install-plugin'],
					'url' => '',
					'type' => 'cta',
				]
			];
		...
		function cfef_dismiss_notice_callback() {
		    ...
		    if ( empty( $nonce ) || empty( $type ) || ! wp_verify_nonce( $nonce, "cfef_dismiss_nonce_{$type}" ) ) {
            wp_send_json_error([ 'message' => 'Invalid nonce' ]);
         }

		 	if ($type === 'cool_form') {
				update_option('cfef_marketing_dismissed', true);
		...
		public function cfef_register_controls($element) {
			...
			$element->add_control(
					...
					[
						'label' => __('Enable Smart Filters','conditional-fields-for-elementor-form'),
						'type' => \Elementor\Controls_Manager::SWITCHER,
						'default' => 'no',
						'label_on' => __('Yes','conditional-fields-for-elementor-form'),
						'label_off' => __('No','conditional-fields-for-elementor-form'),
						'return_value' => 'yes',
						'condition' => [
							'selected_element!' => '',
						],
			...
			$element->add_control(
					...
					[
						...
						'raw'       => '<div class="elementor-control-raw-html cool-form-wrp"><div class="elementor-control-notice elementor-control-notice-type-info">
										...
										<img class="cfef-highlight-icon" src="'.esc_url( CFEF_PLUGIN_URL . 'admin/marketing/images/cfef-highlight-icon.svg' ).'" width="250" alt="Highlight Icon" />
										</div>
										...
										<div class="elementor-control-notice-main-content">Enable smart taxonomy filters for your Elementor loop grid.</div>
											...
											<button type="button" class="elementor-button e-btn e-info e-btn-1 cfef-install-plugin"  data-plugin="loop-grid" data-nonce="' . esc_attr(wp_create_nonce('cfef_install_nonce')) . '">Install Loop Grid Extender</button></button>
										</div></div>
										</div></div>',
		...
		function cfef_show_tec_active_notice(){
			...
			if (
				...
				|| get_option('cfef_tec_notice_dismissed')
			) {
				return;
			}
			...
			if ( ($is_tribe_post || $is_tec_settings) && !isset($_GET['taxonomy']) ) {
				...
				?>
								<div class="notice notice-info is-dismissible cfef-tec-notice"
									data-notice="tec_notice"
									data-nonce="<?php echo esc_attr(wp_create_nonce('cfef_dismiss_nonce_tec_notice')); ?>">

									<p class="ect-notice-widget">
										<button class="button button-primary cfef-install-plugin"
											data-plugin="events-widget"
											data-notice="tec_notice"
											data-nonce="<?php echo esc_attr(wp_create_nonce('cfef_install_nonce')); ?>">
		...
		public function cfef_init_hooks() {
		...
		public function cfef_add_acf_repeater_mkt_query_controls($element) {
			...
			$element->add_control(
				...
				'lgefep_mkt_country_notice',
					array(
						'name'            => 'cfef_mkt_country_notice',
						'type'            => \Elementor\Controls_Manager::SWITCHER,
						'label'        => esc_html__('Use ACF Repeater','conditional-fields-for-elementor-form'),
						'type'         => \Elementor\Controls_Manager::SWITCHER,
			...
			$element->add_control(
				...
				'lgefep_acf_mkt_repeater_tag',
					[
						...
						'type'      => \Elementor\Controls_Manager::RAW_HTML,
							'raw'       => '<div class="elementor-control-raw-html cool-form-wrp"><div class="elementor-control-notice elementor-control-notice-type-info">
											<div class="elementor-control-notice-icon"><img class="cfef-highlight-icon" src="'.esc_url( CFEF_PLUGIN_URL . 'admin/marketing/images/cfef-highlight-icon.svg' ).'" width="250" alt="Highlight Icon" />
											</div>
											<div class="elementor-control-notice-main">
		...
		public function enqueue_editor_styles(){
			...
			wp_enqueue_style(
				...
				CFEF_PLUGIN_URL . 'admin/marketing/css/cfef-mkt.css',
				[],
				CFEF_VERSION
			);
		}
		 ...
		 */
		public function cfef_install_plugin() {


             if (! current_user_can('install_plugins')) {
				$status['errorMessage'] = __('Sorry, you are not allowed to install plugins on this site.','conditional-fields-for-elementor-form');
				wp_send_json_error($status);
			}

			check_ajax_referer('cfef_install_nonce');

			if (empty($_POST['slug'])) {
				wp_send_json_error(array(
					'slug'         => '',
					'errorCode'    => 'no_plugin_specified',
					'errorMessage' => __('No plugin specified.' ,'conditional-fields-for-elementor-form'),
				));
			}
				...
				if (is_wp_error($result)) {
					...
					if ($skin->result->get_error_message() === 'Destination folder already exists.') {
						...
						if (current_user_can('activate_plugin', $install_status['file'])) {
							$this->cfef_set_install_by_option( $plugin_slug );
							$network_wide = (is_multisite() && 'import' !== $pagenow);
					...
					$status['errorMessage'] = __('Unable to connect to the filesystem. Please confirm your credentials.','conditional-fields-for-elementor-form');
		...
		private function cfef_set_install_by_option( $plugin_slug ) {
		 ...
		 */
		public function cfef_marketing_controls($widget) {
			...
			$dismissed = get_option('cfef_marketing_dismissed', false);
			...
			if (! $dismissed) {
					...
					$widget->add_control(
						'cfef_marketing_box',
						[
							'name'      => 'cfef_marketing_box',
							'label'     => '',
							'type'      => \Elementor\Controls_Manager::RAW_HTML,
							'raw'       => '<div class="elementor-control-raw-html cool-form-wrp"><div class="elementor-control-notice elementor-control-notice-type-info">
											<div class="elementor-control-notice-icon"><img class="cfef-highlight-icon" src="'.esc_url( CFEF_PLUGIN_URL . 'admin/marketing/images/cfef-highlight-icon.svg' ).'" width="250" alt="Highlight Icon" /></div>
											<div class="elementor-control-notice-main">
												
												<div class="elementor-control-notice-main-content">Add advanced fields & features to your Elementor forms.</div>
												<div class="elementor-control-notice-main-actions">
												<button type="button" class="elementor-button e-btn e-info e-btn-1 cfef-install-plugin" data-plugin="cool-form-lite" data-nonce="' . esc_attr(wp_create_nonce('cfef_install_nonce')) . '">Install Cool FormKit</button></button>
											</div></div>
											<button class="elementor-control-notice-dismiss tooltip-target cfef-dismiss-cross cfef-dismiss-notice" data-notice="cool_form" data-nonce="' . esc_attr(wp_create_nonce('cfef_dismiss_nonce_cool_form')) . '">
			...
			$marketing_notice_controls = array(
				...
				'cfef-mkt-country-conditions' => array(
						'name'         => 'cfef-mkt-country-conditions',
						'label'        => esc_html__('Enable Country Code','conditional-fields-for-elementor-form'),
						'type'         => \Elementor\Controls_Manager::SWITCHER,
						'label_on'     => esc_html__('Yes','conditional-fields-for-elementor-form'),
						'label_off'    => esc_html__('No','conditional-fields-for-elementor-form'),
						'condition'    => array(
				...
				'cfef_mkt_country_notice' => array(
						...
						'raw'             => '<div class="elementor-control-raw-html cool-form-wrp"><div class="elementor-control-notice elementor-control-notice-type-info">
											...
											<div class="elementor-control-notice-main-actions">
											<button type="button" class="elementor-button e-btn e-info e-btn-1 cfef-install-plugin"  data-plugin="country-code" data-nonce="' . esc_attr(wp_create_nonce('cfef_install_nonce')) . '">Install Country Code</button>
						...
						'condition'       => array(
							...
							'field_type' =>  array('tel', 'ehp-tel'),
							'cfef-mkt-country-conditions' => 'yes'
						),
						'inner_tab'       => 'form_fields_content_tab',
						'tabs_wrapper'    => 'form_fields_tabs',
					)
	...
	CFEF_Marketing_Controllers::get_instance();
}
