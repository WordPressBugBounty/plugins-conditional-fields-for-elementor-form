	...
	*/
	function enqueue_feedback_scripts() {
		...
		if ( isset( $screen ) && $screen->id == 'plugins' ) {
			...
			wp_enqueue_style( 'cool-plugins-feedback-css', $this->plugin_url . 'admin/feedback/css/admin-feedback.css', null, $this->plugin_version , false);
		}
			...
			wp_enqueue_script( 'cfef-admin-review-notice-js', $this->plugin_url . 'admin/feedback/js/cfef-admin-review-notice.js', array( 'jquery' ), $this->plugin_version , false);
	...
	foreach ( $active_plugins as $plugin_path ) {
		$plugin_info = get_plugin_data(
			WP_PLUGIN_DIR . '/' . sanitize_text_field( $plugin_path ),
			false,
			false
		);
	...
	public function show_deactivate_feedback_popup() {
		...
		$deactivate_reasons = array(
			'didnt_work_as_expected'         => array(
				'title'             => __( 'The plugin didn\'t work as expected.','conditional-fields-for-elementor-form' ),
				'input_placeholder' => 'What did you expect?',
			),
			'found_a_better_plugin'          => array(
				'title'             => __( 'I found a better plugin.','conditional-fields-for-elementor-form' ),
				'input_placeholder' => __( 'Please share which plugin.','conditional-fields-for-elementor-form' ),
			),
			'couldnt_get_the_plugin_to_work' => array(
				'title'             => __( 'The plugin is not working.','conditional-fields-for-elementor-form' ),
				'input_placeholder' => 'Please share your issue. So we can fix that for other users.',
			),
			'temporary_deactivation'         => array(
				'title'             => __( 'It\'s a temporary deactivation.','conditional-fields-for-elementor-form' ),
				'input_placeholder' => '',
			),
			'other'                          => array(
				'title'             => __( 'Other reason.','conditional-fields-for-elementor-form' ),
				'input_placeholder' => __( 'Please share the reason.','conditional-fields-for-elementor-form' ),
			),
		);
		...
		<div id="cool-plugins-feedback-<?php echo esc_attr( $this->plugin_slug ); ?>" class="hide-feedback-popup">
			...
			<div class="cp-feedback-header">
				<div class="cp-feedback-title"><?php echo esc_html__( 'Quick Feedback','conditional-fields-for-elementor-form' ); ?></div>
			...
			<div class="cp-feedback-form-wrapper">
				<div class="cp-feedback-form-title"><?php echo esc_html__( 'If you have a moment, please share the reason for deactivating this plugin.','conditional-fields-for-elementor-form' ); ?></div>
	...
	function submit_deactivation_response() {
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash($_POST['_wpnonce']) ), '_cool-plugins_deactivate_feedback_nonce' ) || ! current_user_can( 'manage_options' )) {
			...
			$deactivate_reasons = array(
				...
				'other'                          => array(
					'title'             => __( 'Other','conditional-fields-for-elementor-form' ),
					'input_placeholder' => __( 'Please share the reason.','conditional-fields-for-elementor-form' ),
				),
			);
